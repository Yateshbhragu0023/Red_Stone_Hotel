            ...
            >
                {reviews.map((review, index) => (
                    <SwiperSlide key={index}>
                        <div className="backdrop-blur-xl bg-white/10 border border-white/20 shadow-xl rounded-3xl p-6 h-full flex flex-col justify-between text-center transition-transform transform hover:-translate-y-2 hover:shadow-2xl duration-500">
                            <img
                                src={review.img}
                                alt={review.name}
                                className="w-20 h-20 rounded-full mx-auto border-4 border-[#AB854F] mb-4"
                            />
                            <p className="text-gray-100 italic mb-6">“{review.text}”</p>
                            <div>
